...
#include "infocoinaddressvalidator.h"
...
{
    widget->setMaxLength(InfocoinAddressValidator::MaxAddressLength);
    widget->setValidator(new InfocoinAddressValidator(parent));
    widget->setFont(infocoinAddressFont());
}
...
bool parseInfocoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // NovaCoin: check prefix
    if(uri.scheme() != QString("infocoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!InfocoinUnits::parse(InfocoinUnits::INFO, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseInfocoinURI(QString uri, SendCoinsRecipient *out)
{
    // Convert infocoin:// to infocoin:
    //
    //    Cannot handle this later, because infocoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("infocoin://"))
    {
        uri.replace(0, 12, "infocoin:");
    }
    QUrl uriInstance(uri);
    return parseInfocoinURI(uriInstance, out);
}
...
void openConfigfile()
{
    ...
    /* Open infocoin.conf with the associated application */
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a infocoin.desktop file to the autostart directory:
