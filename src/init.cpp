...
void Shutdown()
{
    ...
    if (pwalletMain)
        bitdb.Flush(true);
    GenerateInfocoins(false, NULL, 0);
#endif
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: infocoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: infocoind.pid)") + "\n";
...
/** Sanity checks
 *  Ensure that Infocoin is running in a usable environment with all
...
bool InitSanityCheck(void)
{
    if(!ECC_InitSanityCheck()) {
        InitError("OpenSSL appears to lack support for elliptic curve cryptography. For more "
                  "information, visit https://en.infocoin.it/wiki/OpenSSL_and_EC_Libraries");
        return false;
    }
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Infocoin is shutting down."));
...
#endif
    // Make sure only a single Infocoin process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Infocoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Infocoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    // requested to kill infocoin-qt during the last operation. If so, exit.
    ...
    if (pwalletMain)
        GenerateInfocoins(GetBoolArg("-gen", false), pwalletMain, GetArg("-genproclimit", -1));
#endif
