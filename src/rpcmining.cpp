...
Value setgenerate(const Array& params, bool fHelp)
{
    ...
    GenerateInfocoins(fGenerate, pwalletMain, nGenProcLimit);
    return Value::null;
}
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Infocoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Infocoin is downloading blocks...");
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "See https://en.infocoin.it/wiki/BIP_0022 for full specification.");
