...
void static InfocoinMiner(CWallet *pwallet)
{
    LogPrintf("InfocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("infocoin-miner");
	...
	CBlock *pblock = pblocktemplate.get();
        ...
        LogPrintf("Running InfocoinMiner with %llu transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("InfocoinMiner terminated\n");
        throw;
    }
}

void GenerateInfocoins(bool fGenerate, CWallet* pwallet, int nThreads)
{
	...
	minerThreads->create_thread(boost::bind(&InfocoinMiner, pwallet));
}
#endif
