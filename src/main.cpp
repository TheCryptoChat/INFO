...
#if defined(NDEBUG)
# error "Infocoin cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge = 12 * 60 * 60; // 12 hour(s)
...
unsigned int DarkGravityWave(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Modified for Infocoin: softfork block set to min diff to re-engage chain movement
...
void VRX_ThreadCurve(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Version 1.0
    //
    int64_t nNow = nBestHeight; int64_t nThen = VRX_FDIFF; // Toggle skew system fork
    ...
    //
    if(pindexBest->GetBlockTime() > VRX_FDIFF) // ON (TOGGLED Nov/01/2017)
    {
        // Define time values
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal infocoin miner
...
#ifdef ENABLE_WALLET
// infocoin: attempt to generate suitable proof-of-stake
